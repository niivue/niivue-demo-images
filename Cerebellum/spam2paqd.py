...
if len(sys.argv) < 2 or not sys.argv[1].endswith(('.nii', '.nii.gz')):
    print("Usage: python spam2paqd.py input_filename.nii[.gz] [--normalize]")
    sys.exit(1)
...
data = nii.get_fdata()
data[~np.isfinite(data)] = 0.0  # Replace NaN/inf with 0.0

affine = nii.affine

# Normalize if requested
max_val = np.max(data)
print(f"Maximum voxel value before normalization: {max_val}")
if normalize and max_val > 0:
    data = data / max_val
    print("Data was normalized to the range [0, 1]")
else:
    print("Data was left in original scale")
